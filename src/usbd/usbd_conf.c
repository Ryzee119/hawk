...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  uint32_t base = 8 * MAX((sizeof(hpcd_USB_FS.IN_ep) / sizeof(*hpcd_USB_FS.IN_ep)), (sizeof(hpcd_USB_FS.OUT_ep) / sizeof(*hpcd_USB_FS.OUT_ep)));

  HAL_PCDEx_PMAConfig((PCD_HandleTypeDef*)pdev->pData, 0x00, PCD_SNG_BUF, base += USB_MAX_EP0_SIZE);
  HAL_PCDEx_PMAConfig((PCD_HandleTypeDef*)pdev->pData, 0x80, PCD_SNG_BUF, base += USB_MAX_EP0_SIZE);

  uint32_t out = (base + 64 * 0) | ((base + 64 * 1) << 16);
  uint32_t in = (base + 64 * 2) | ((base + 64 * 3) << 16);

  HAL_PCDEx_PMAConfig((PCD_HandleTypeDef*)pdev->pData, USBD_XBLC_OUT_EP, PCD_DBL_BUF, out);
  HAL_PCDEx_PMAConfig((PCD_HandleTypeDef*)pdev->pData, USBD_XBLC_IN_EP, PCD_DBL_BUF, in);
